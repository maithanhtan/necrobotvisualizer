...
Utils.CalculateMaxCpCurrentLvl = function(pkm) {
  return Math.max(
    parseInt(Math.floor(
      0.1*Utils.CalculateMaxCpMultiplier(pkm)*
      Math.pow(pkm.CpMultiplier + pkm.AdditionalCpMultiplier, 2)
    )),
    10
  );
}
Utils.CalculatePerfectCpMaxLvl = function(pkm) {
  var bs = Utils.baseStats[pkm.PokemonId];
  
  return parseInt((bs[1] + 15)
    *Math.sqrt(bs[2] + 15) 
    *Math.sqrt(bs[0] + 15)
    *Math.pow(Utils.CpM[39],2) / 10);
}
Utils.CalculateMaxCpMaxLvl = function(pkm) {
  var bs = Utils.baseStats[pkm.PokemonId];
...
Utils.CalculateMaxCpMultiplier = function(pkm) {
  var bs = Utils.baseStats[pkm.PokemonId];
  
  return (bs[1] + 15)*Math.sqrt(bs[2] + 15)*Math.sqrt(bs[0] + 15);
}
Utils.CalculateMaxLevelItems = function(pkm) {
  var level = parseInt(Utils.GetLevel(pkm))*2;
  var dust = 0, candy = 0;
